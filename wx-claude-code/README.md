...
- `modelos-locais`: rodar o trabalho mecânico num modelo local pelo
  [Magnitude](https://github.com/magnitudedev/magnitude) (Apache 2.0, instalado
  por npm, não vem no plugin). Liga em `J.modelos_locais`; regra, decisão e
  prova nunca vão para lá.
...
biblioteca padrão. **Faltando algum, o instalador mostra o comando que o
resolveria e pergunta antes de rodar** — nada é instalado sem você aprovar. A lista completa, com o que é opcional e o que se perde sem
cada coisa, está em `PRE-REQUISITOS.md`.
...
- **UI/UX Pro Max** (7 skills vendorizadas de
  [nextlevelbuilder/ui-ux-pro-max-skill](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill),
  MIT): `ui-ux-pro-max` com base local pesquisável de estilos, paletas, pares de
  fontes e diretrizes de UX, mais `design`, `design-system`, `ui-styling`,
  `banner-design`, `brand` e `slides`. Servem à tela do destino, ao lado do
  Impeccable. Atribuição em `skills/ui-ux-pro-max/NOTICE.md`.
...
## PDF citável e registro de operações

`/wx-claude-code:pdf` converte um PDF em Markdown com uma seção por página,
SHA-256 no cabeçalho e página sem texto marcada `OCR_REQUERIDO` em vez de
inventada (skill `pdf-para-markdown`).

`/wx-claude-code:log` mostra o registro: toda operação do plugin grava uma
linha em `.wx-migration/logs/`, inclusive as negativas dos hooks. Sem projeto
por perto não grava nada, falha de registro não derruba a operação, e segredo
nunca entra.

## As respostas ficam em markdown, para os agentes

`.wx-migration/respostas_questionario.md` traz as **60 respostas** com índice por id
e o estado de cada uma. É onde um agente procura antes de perguntar: resposta
vazia aparece como «não respondido», e só então se pergunta aquele item com
`/wx-claude-code:pergunta <id>`. Regravado a cada aplicação, indexado pelo RAG.
