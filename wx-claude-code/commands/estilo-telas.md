...
2. Execute `init` do Impeccable a partir do `PRODUCT.md` que o questionário já deixou (F1: quem opera, horas, ambiente, tela típica; modo *Operate*) (público, propósito, plataformas de **I**, restrições). Não pergunte de novo o que já foi respondido. Isso escreve `PRODUCT.md`.
...
5. Rode um comando do Impeccable por seção respondida do `DESIGN.md`, nesta ordem, e nenhum para seção sem resposta (aí é pergunta): `shape` nas grids (F3), `harden` em formulários e estados (F4, F7), `typeset` nos formatos (F5), `layout` na impressão (F6), `audit` na acessibilidade (F8), `adapt` para a tela típica de F1. Registre o resultado de cada um como `DEC-*` ou `GAP-*`.
