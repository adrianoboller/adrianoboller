...
const scenes = [
  ...
  ['/wx-claude-code:questionario · letra H · processo', '6 · Como seria a conversão: o que cada peça do WX vira na linguagem escolhida, e depois a estratégia', cap('processo').split('\n').slice(17).join('\n')],
  ['exemplos/estoque-wx', '7 · Projeto de exemplo real: G0 sem erros, texto com localizador, golden master 9/10', cap('exemplo').split('\n').slice(14).join('\n')],
  ['DESIGN.md · letra F', '8 · Qualidade de ERP: treze subperguntas viram a tabela de botões, posição, ícone, cor e fundo', cap('design-erp').split('\n').slice(12, 48).join('\n')],
  ['query_wlanguage_help.py', '9 · O corpus WLanguage 12k, verificado por hash e consultado por tema', cap('help').split('\n').slice(0, 24).join('\n') + '\n…'],
  ['subagentes wl-*-specialist', '10 · Cada símbolo vai ao especialista WLanguage do tema certo do Help', cap('equipe')],
  ['/wx-claude-code:pmo', '11 · PMO: sprint Scrum, ciclos PDCA e a base de conhecimento', cap('pmo2').split('\n').slice(0, 33).join('\n')],
