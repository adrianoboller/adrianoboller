...
def medir() -> dict:
    ...
    dur_bateria = duracao("wx-claude-code-video-bateria.mp4")
    cenas_bateria = cenas_de("bateria")
    dur_primeiro = duracao("wx-claude-code-video-primeiro.mp4")
    cenas_primeiro = cenas_de("primeiro")
    dur_windev = duracao("wx-claude-code-video-windev.mp4")
    cenas_windev = cenas_de("windev")
    dur_passos = duracao("wx-claude-code-video-passos.mp4")
    cenas_passos = cenas_de("passos")
    dur_ativacao = duracao("wx-claude-code-video-ativacao.mp4")
    cenas_ativacao = cenas_de("ativacao")
    # os destinos provados: cada exemplo com destino/ traz o golden comparado; o numero sai do relatorio
    destinos = []
    for comp in sorted(RAIZ.glob("exemplos/*/destino/golden-master/comparacao.json")):
        d = json.loads(comp.read_text(encoding="utf-8"))
        destinos.append({"exemplo": comp.parents[2].name, "golden": d.get("equivalencia", "INDISPONÍVEL")})
    testes_wl_rt = sum(p.read_text(encoding="utf-8").count("#[test]") for p in RAIZ.glob("ferramentas/wl-rt/src/*.rs"))
    wl_csharp = json.loads((RAIZ / "skills/conversao-wx/resources/wl-csharp/funcoes.json").read_text(encoding="utf-8"))
    ...
    return {
        ...
        "prints": conta("docs/prints/*.png"), "video_duracao": dur, "video_cenas": cenas, "video_php_duracao": dur_php, "video_php_cenas": cenas_php, "video_bateria_duracao": dur_bateria, "video_bateria_cenas": cenas_bateria, "video_primeiro_duracao": dur_primeiro, "video_primeiro_cenas": cenas_primeiro, "video_windev_duracao": dur_windev, "video_windev_cenas": cenas_windev, "video_passos_duracao": dur_passos, "video_passos_cenas": cenas_passos, "video_ativacao_duracao": dur_ativacao, "video_ativacao_cenas": cenas_ativacao, "destinos_provados": len(destinos), "golden_dos_destinos": " · ".join(f"{d['golden']} ({d['exemplo']})" for d in destinos), "destinos": destinos, "testes_wl_rt": testes_wl_rt, "funcoes_wl_csharp": wl_csharp["quantidade"], "classes_wl_csharp": len(wl_csharp["classes"]), "tipos_wl_csharp": wl_csharp["quantidade_tipos_avancados"], "constantes_wl_csharp": sum(len(v) for v in wl_csharp["constantes"].values()),
...
def relatorio(n: dict) -> str:
    ...
    rot = {"agentes": "agentes", "papeis": "papéis A–J", "subagentes_pdca": "subagentes PDCA", "especialistas_wl": "especialistas WLanguage por tema", "comandos": "comandos /", "skills": "skills", "skills_erp": "skills de ERP (pacote skills.sh)", "scripts": "scripts Python", "linhas_de_python": "linhas de Python (scripts e hooks)", "referencias": "documentos de referência", "testes": "testes de regressão", "hooks": "hooks do plugin", "blocos_do_questionario": "blocos do questionário (0, A–M)", "itens_do_bloco_0": "itens do bloco 0", "subperguntas_f": "subperguntas de F (F0–F13)", "itens_k": "itens de K", "itens_l": "itens de L", "arquivos_gerados_pelo_questionario": "arquivos que o questionário pode gerar", "prints": "prints de sessões reais", "video_cenas": "cenas do vídeo", "video_duracao": "duração do vídeo", "video_php_cenas": "cenas do vídeo de PHP para Rust", "video_php_duracao": "duração do vídeo de PHP para Rust", "video_bateria_cenas": "cenas do vídeo da bateria de testes", "video_bateria_duracao": "duração do vídeo da bateria de testes", "video_primeiro_cenas": "cenas do vídeo do primeiro projeto", "video_primeiro_duracao": "duração do vídeo do primeiro projeto", "video_windev_cenas": "cenas do vídeo dos PDFs do WINDEV a Rust + React", "video_windev_duracao": "duração do vídeo dos PDFs do WINDEV a Rust + React", "video_passos_cenas": "cenas do vídeo passo a passo", "video_passos_duracao": "duração do vídeo passo a passo", "video_ativacao_cenas": "cenas do vídeo da ativação", "video_ativacao_duracao": "duração do vídeo da ativação", "destinos_provados": "projetos de exemplo convertidos de ponta a ponta", "golden_dos_destinos": "golden master dos destinos", "testes_wl_rt": "testes do runtime WLanguage em Rust (wl-rt)", "funcoes_wl_csharp": "funções da WL_C# 1.2 no índice (lidas do metadado do WL.dll)", "classes_wl_csharp": "classes da WL_C# no índice", "tipos_wl_csharp": "tipos avançados da WL_C# no índice", "constantes_wl_csharp": "constantes da WL_C# no índice", "corpus_bytes": "corpus do Help (bytes)", "corpus_paginas_validas": "páginas válidas do corpus", "manual_linhas": "linhas do manual", "exemplo_tabelas": "tabelas do exemplo ESTOQUE"}
    ...
    L += ["", "## O que foi provado em sessão real", "", "Cada print em `docs/prints/` é a saída de uma sessão do Claude Code ou de um script, sem edição; a origem de cada um está em `docs/prints/gerar.md`. Entre eles: o questionário uma letra por vez, a senha colada que não é gravada nem repetida, a letra H com o processo de conversão, a tela modelo aberta antes de registrar, o serial de ativação recusando e depois liberando, a primeira sessão lendo `INDEX_FILES.md` e o kickoff, a exportação organizada e o zelador, e o esqueleto de ERP (L6) com a sessão carregando a skill do módulo.", "",
          ...
          "## Onde está cada coisa", "", "- Manual: `MANUAL.md` (PDF em `docs/manual-de-uso.pdf`); oito capítulos.", "- Página para investidores: `docs/investidor/`.", "- Análise da aula de vibe coding: `docs/analise-aula-vibe-coding.md`.", "- Telas do fluxo de licença: `docs/telas-licenca/`.", "- Segurança (o que protege e o que não): `docs/SEGURANCA.md`.", "- Emissor de serial, fora do plugin: `ferramentas/wx-serial/`.", "- Dossiê: `docs/dossie/dossie-wx-claude-code.html`, gerado deste mesmo medidor.", ""]
    return "\n".join(L)
