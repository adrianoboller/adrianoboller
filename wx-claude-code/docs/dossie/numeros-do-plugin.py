...
def medir() -> dict:
    ...
    try:
        ...
        out = subprocess.run([imageio_ffmpeg.get_ffmpeg_exe(), "-i", str(video)], capture_output=True, text=True).stderr
        m = re.search(r"Duration: (\d+):(\d+):(\d+)", out)
        if m:
            dur = f"{int(m.group(2))} min {int(m.group(3)):02d} s"
    except Exception:
        dur = "INDISPONÍVEL"
    cenas = len(re.findall(r"'\d+ · ", (RAIZ / "docs/video/gravar-video.mjs").read_text(encoding="utf-8")))
    ...
    return {
        ...
        "prints": conta("docs/prints/*.png"), "video_duracao": dur, "video_cenas": cenas,
