...
fluxo = ["Questionário<br><small>bloco 0, A–L</small>", "Pré-flight G0<br><small>anexos conferidos</small>", "Inventário G1–G3<br><small>matriz, decisões</small>", "Piloto G4<br><small>golden master</small>", "Ondas G5–G6<br><small>papéis A–J, PDCA</small>", "Cutover G7<br><small>relatório, exportação</small>"]
...
faltas = ["Nenhum projeto WINDEV real passou pelos gates G1 a G7 de ponta a ponta; o exemplo ESTOQUE é sintético.", "Os scripts de ambiente são bash; não há PowerShell, e o público usa Windows.", "A licença é dissuasão por hook; servir corpus e agentes de um servidor ficou para depois, por decisão do dono.", "O custo em tokens do questionário inteiro numa sessão real não foi medido.", "O questionário não pausa nem retoma; com mais de setenta itens, isso pesa."]
faltas_html = "".join(f"<li>{E(f)}</li>" for f in faltas)
mapa = [("MANUAL.md, docs/manual-de-uso.pdf", "manual de oito capítulos"), ("docs/relatorio-do-plugin.md", "relatório medido do estado atual"), ("docs/investidor/", "página e PDF para investidores"), ("docs/prints/, docs/video/", "provas reais e o vídeo de uso"), ("docs/analise-aula-vibe-coding.md", "o que a aula de vibe coding ensinou e o que não se copiou"), ("docs/telas-licenca/", "sete telas do fluxo de liberação de licença"), ("licenca/LEIA-ME.md", "o que o serial protege e o que não"), ("skills/LEIA-ME-erp.md", "as oito skills de ERP do pacote skills.sh e o esqueleto que L6 gera"), ("exemplos/estoque-wx/", "projeto de exemplo e teste de regressão do fluxo inteiro"), ("tests/testes.py", f"{n['testes']} testes; o validador estrito os roda")]
...
.fluxo{{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:8px}}
@media(max-width:760px){{.fluxo{{grid-template-columns:repeat(3,minmax(0,1fr))}}}}
