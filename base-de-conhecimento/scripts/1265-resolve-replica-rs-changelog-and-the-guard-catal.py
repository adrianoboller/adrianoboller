# Resolve replica.rs, CHANGELOG and the guard catalog
# 30/08 06:35

# replica.rs: fica o lado da cifra (o canal), porque o teto agora mora dentro
# dele -- e a constante local perde a razao de existir.
p='crates/phxsql-server/src/replica.rs'
ls=open(p,encoding='utf-8').read().split('\n'); out=[];i=0;c=0
while i<len(ls):
    if ls[i].startswith('<<<<<<<'):
        i+=1
        while not ls[i].startswith('======='): i+=1
        i+=1; z=[]
        while not ls[i].startswith('>>>>>>>'): z.append(ls[i]); i+=1
        i+=1; out.extend(z); c+=1
    else: out.append(ls[i]); i+=1
open(p,'w',encoding='utf-8').write('\n'.join(out))
print(f"replica.rs: {c} conflito, lado da cifra")

for cam in ['CHANGELOG.md']:
    ls=open(cam,encoding='utf-8').read().split('\n'); out=[];i=0;c=0
    while i<len(ls):
        if ls[i].startswith('<<<<<<<'):
            i+=1;a=[]
            while not ls[i].startswith('======='): a.append(ls[i]);i+=1
            i+=1;z=[]
            while not ls[i].startswith('>>>>>>>'): z.append(ls[i]);i+=1
            i+=1;out.extend(a);out.extend(z);c+=1
        else: out.append(ls[i]);i+=1
    open(cam,'w',encoding='utf-8').write('\n'.join(out))
    print(f"{cam}: {c} conflitos, dois lados")
