...
meu, dele = ler('HEAD'), ler('worktree-agent-a920a25542813ad28')
...
faltando=[]
for nome in sorted(set(re.findall(r'\b([A-Z][A-Z0-9_]{4,})\b', ''.join(blocos)))):
...
print("constantes junto:", [f.split('=')[0].strip() for f in faltando])
corpo=meu.rstrip()[:-1].rstrip('\n')
if faltando:
    marca=re.search(r'^[A-Z_]+\s*(?::[^=]+)?=\s*\[', corpo, re.M)
