...
cargas = {
 "csv":  "id;nome;cidade;limite;ativo;cadastro\n1;Adriano;Blumenau;1.500,50;sim;2026-08-28\n"
         "2;\"Silva, Souza & Cia\";Itajai;;nao;2026-01-15\n",
 "txt":  "id\tnome\tlimite\tativo\n1\tAdriano\t1500.50\ttrue\n",
 "json": '[{"id":1,"nome":"Adriano","limite":"1500.50","ativo":true,"cadastro":"2026-03-01"}]',
 "xml":  '<?xml version="1.0"?><linhas><linha><id>1</id><nome>Silva &amp; Souza</nome>'
         '<limite>99,90</limite><ativo>1</ativo></linha></linhas>',
...
for fmt, texto in cargas.items():
    ...
    r=op({"op":"inserir_lote","database":"loja","tabela":tab,"texto":texto}).get("resultado",{})
    ...
    for l in v["linhas"]:
        print("     ", {k:v2 for k,v2 in l.items() if k in ("id","nome","limite","ativo","cadastro")})
