...
pub struct FioRepasse {
    ...
    chave_no: [u8; 32],
    estado: Mutex<Estado>,
    /// Metade de escrita da conexao TCP (quem cifra manda por aqui).
    escrita: Mutex<Option<TcpStream>>,
...
impl FioRepasse {
    ...
    /// Chamado a cada segundo: decide a troca de fio no `Auto` e larga a
    /// conexao TCP que nao confirma.
    pub fn vigiar(&self) {
        let mut e = self.estado.lock().expect("fio");
    ...
    fn largar(&self) {
        if let Some(s) = self.escrita.lock().expect("escrita").take() {
            let _ = s.shutdown(Shutdown::Both);
        }
    }
        ...
        if l.is_none() || !viva {
                ...
                .and_then(|s| s.try_clone().map(|c| (s, c)).map_err(|e| e.to_string()))
            {
                Ok((s, c)) => {
                    ...
                    *self.escrita.lock().expect("escrita") = Some(c);
