#!/usr/bin/env bash
# Sobe um painel descartavel (PostgreSQL proprio, sem OpenVPN) e exercita a
# tela do autenticador no Chromium (tela.mjs). Capturas em SAIDA.
# Uso: sudo ./tela.sh [pasta]
set -euo pipefail
AQUI=$(cd "$(dirname "$0")" && pwd)
RAIZ=$(cd "$AQUI/../.." && pwd)
T=${1:-$(mktemp -d /tmp/phxvpn-tela-mfa.XXXX)}
BIN=${PHXVPN_BIN:-$RAIZ/target/debug/phxvpn}
PGBIN=$(ls -d /usr/lib/postgresql/*/bin | sort -V | tail -1)
PGPORTA=55493; PAINEL=127.0.0.1:8481
