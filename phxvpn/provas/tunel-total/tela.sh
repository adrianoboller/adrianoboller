#!/usr/bin/env bash
# Sobe um painel descartavel (PostgreSQL proprio, sem OpenVPN: o firewall nao
# se aplica e a tela tem de DIZER isso) e exercita a saida da rede (tunel total e DNS) no
# Chromium (tela.mjs). Capturas e o resumo em SAIDA; o resumo entra na chave
# "tela" do resultados.json ao lado.
# Uso: sudo ./tela.sh [pasta]
set -euo pipefail
AQUI=$(cd "$(dirname "$0")" && pwd)
RAIZ=$(cd "$AQUI/../.." && pwd)
T=${1:-$(mktemp -d /tmp/phxvpn-tela-tt.XXXX)}
BIN=${PHXVPN_BIN:-$RAIZ/target/debug/phxvpn}
PGBIN=$(ls -d /usr/lib/postgresql/*/bin | sort -V | tail -1)
PGPORTA=55496; PAINEL=127.0.0.1:8483
...
TA=$(api POST /api/login "" '{"usuario":"ana","senha":"senha-da-ana-longa"}' | python3 -c "import json,sys; print(json.load(sys.stdin)['token'])")
api POST /api/redes/entrar "$TA" '{"nome":"Matriz","senha":"senha-da-rede"}' >/dev/null
PAINEL="http://$PAINEL/" SAIDA=$T /opt/node22/bin/node "$AQUI/tela.mjs" | tee "$T/tela.json"
python3 - "$AQUI/resultados.json" "$T/tela.json" <<'PY'
import json, sys, os
caminho, tela = sys.argv[1], json.load(open(sys.argv[2]))
...
sys.exit(0 if tela.get("passou") else 1)
PY
