...
| `saude-do-disco` | o cartão da **saúde do disco** no Painel (pedido 249), nos dois estados: saudável com a sonda que subiu junto do servidor, e **com erro** quando o sistema operacional recusa a sonda de verdade — o caso põe um diretório no lugar do canário `.saude-do-disco` e o `open` cai com `EISDIR (21)`. Lê o `data-estado` do pino, nunca a frase; confere que o pino carrega **forma** (●/✕) além de cor; e confere a volta a saudável na passada seguinte |
...
| `botoes-do-assistente-de-replicacao` | a navegação inteira do assistente de replicação **sem um segundo servidor**: o assistente aponta a sonda para o próprio servidor da bateria — `replicacao_testar` é um cliente do protocolo, e 127.0.0.1 na porta de dados dela responde de verdade. Doze dos dezenove botões saem disso, e mede onde ele de fato para: o passo 4 em diante exige uma sonda **sem impedimento**, e o servidor da bateria se reprova como origem por não declarar `id_servidor` nem `imagem_da_linha` |
