...
| `Encryption` | a cifra do fio | ✔ **existe** — X25519 + HKDF-SHA256 + ChaCha20-Poly1305, aperto estilo Noise `NX` (`docs/CIFRA-DO-FIO.md`). ✖ **dispensa como `ALTER CONNECTION SET`**: ela se negocia no **aperto de mão** (op `cifrar`), antes de haver sessão para configurar. `SHOW CONNECTION SETTINGS` diz a porta por onde a conexão entrou (`via`: `dados`, `http` ou `interna`) e o estado em **três** campos, e a diferença entre eles é o conserto do pedido 370: `encryption` é a **capacidade do servidor** (ele atende o aperto), `encryption_neste_canal` é a verdade **desta** conexão (ela está dentro do túnel?) e `encryption_exigida` diz se o texto claro seria recusado **nesta** conexão — numa porta HTTP os dois últimos são `false`, porque quem cifra ali é o TLS do proxy, que este servidor não tem como conferir. |
