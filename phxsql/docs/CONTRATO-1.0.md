...
| **G** | **Cascata parcial não acontece em silêncio numa QUEDA.** Quando o índice da filha está sujo, a recuperação **recusa** cascatear e denuncia em `operacoes IMPOSSIVEIS`: 21 corridas, 1.200 filhas, 9 casos, 9 denunciados, zero calados. **O que isso NÃO cobre, dito:** na cascata **solta** do `ao_alterar` (fora de transação) não há marca. Um **pânico** no meio dela, desde o pedido 490, deixa o que a queda deixa: o `.ndx` da filha marcado, e a filha **recusando até o `reindexar` enquanto o processo vive**. Mas o arranque (pedido 522) reconstrói sozinho todo `.ndx` marcado, então no primeiro reinício a recusa some e sobra só a contagem «índices reconstruídos» do relatório — e as filhas que ficaram na chave velha continuam lá, sem aviso, na queda e no pânico igual. Completar essa cascata é pedido aberto (P4 do parecer de integridade de 24/09/2026) | `TRANSACOES.md` §5.5.3; o pânico em `SEGURANCA.md` §24.3 e `ACID.md` §2.4 |
