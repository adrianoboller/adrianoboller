...
| **G** | **Cascata parcial não acontece em silêncio numa QUEDA.** Quando o índice da filha está sujo, a recuperação **recusa** cascatear e denuncia em `operacoes IMPOSSIVEIS`: 21 corridas, 1.200 filhas, 9 casos, 9 denunciados, zero calados. **E a cascata SOLTA** do `ao_alterar` (fora de transação) passou a ter marca no pedido 540: pelo servidor, ela é uma transação de uma instrução, e queda, `SIGKILL` ou pânico no meio dela são **completados** — medido com o conserto desligado, `[5, 5]` depois do pânico e `[6, 5]` depois do `SIGKILL`; com ele, `[6, 6]`. **O que isso NÃO cobre, dito:** quem usa o `phxsql-store` embutido e chama o `Table::atualizar` direto continua sem marca — ali vale o 490: a filha recusa até o `reindexar` enquanto o processo vive, o arranque (522) a reconstrói e sobra só a contagem «índices reconstruídos», e as filhas que ficaram na chave velha continuam lá | `TRANSACOES.md` §5.5.3; o pânico em `SEGURANCA.md` §24.3 e `ACID.md` §2.4 |
