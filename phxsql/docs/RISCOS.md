...
| R19 | A dívida marcada no fonte pode ser **apagada sem ser paga**, e nada acusa: a marca é uma linha de comentário em arquivos que receberam **197 commits em 14 dias** (`servidor.rs` 81, `table.rs` 36, `config.rs` 24), e a contagem **melhora** quando ela some. A redação anterior — «a contagem sobe sozinha» — foi **derrubada medida** em 16/09/2026: a série tem **um ponto** (as 19 nasceram juntas em `f64b822`), e subir seria o inventário andando. Um teto pioraria, porque puniria quem marca, e **9 das 19** não se pagam por engenharia (pétrea, recusa certa, decisão do dono) | média | médio | G | 2026-09-16 | divida | `docs/propostas/catraca-da-divida.md` (o parecer que mediu) · `docs/status/riscos.py` (a varredura) · `docs/CATRACAS.md` |
