...
| 10 | **A interface em seis idiomas — 15% dela** | `cargo run --example textos-fora-da-fabrica -p phxsql-server` conta **1.999 ainda cravados em português**, com arquivo e linha de cada um — 1.809 no `index.html`, 126 no `claude.js`, 38 no `telemetria.js`, 24 no `phx-grid.js` e 2 no `diagrama-er.js`. O `multitela.js` está em **zero**: é o primeiro arquivo de interface inteiramente traduzido, e foi ele que ensinou a regra que vale para tudo o que falta. Dos 69 textos cravados dele, 68 viraram 70 chaves de fábrica e o sexagésimo nono (`devicePixelRatio`) entrou nos isentos com a razão escrita. **Trinta e nove dos 68 eram UMA frase picada pela marcação** — `"funcionam em"` + `<b>qualquer navegador</b>` + `"— é layout…"` —, e fragmento não se traduz: a ordem das palavras muda de língua para língua e em alemão o verbo vai para o fim. A frase inteira virou uma chave, com a ênfase como **marca dentro do texto** (`**assim**`, e a palavra entre crases), e o corte em `<b>`/`<code>` passou a acontecer **depois** da tradução, no `marcado()` da página — que escapa tudo antes, porque a célula é editável e célula editável é entrada de usuário. Três testes novos travam o mecanismo (etiqueta crua, marca não fechada, `{n}` perdido numa tradução) e a prova pelo navegador (`prova-idiomas.mjs`, passo 7) abre a tela do modo, troca para alemão e confere que nada ficou em português. Traduzido 100%: a tela de entrada, o cromo inteiro (menu, barra, abas, árvore, painel lateral), a tela de Idiomas, o cabeçalho da tela de Configurações e o modo multitela. Falta o miolo das telas — por ordem do que se vê: os títulos e subtítulos de `folha(`, os cabeçalhos de coluna, os recados de `avisar(` e as perguntas de `confirm(`. A **catraca** do `conferidor.rs` (`TETO`) impede que o número suba, e ela desceu de 2.068 para **1.999** no mesmo commit da tradução. `docs/MENSAGENS.md` |
