...
| ◐ | 113 | **Atacar os 83,5% do `.ndx`** | **medido, e o alvo era outro.** O custo não era de localidade de chave — era **reler e recalcular CRC-32 da mesma página**: 10,86 páginas tocadas por linha, 8,80 delas releituras, 2,34 µs de CRC cada. Um **cache de páginas de leitura** no `.ndx` levou a inserção de **44,4 → 18,5 µs (2,40×)**, e o cabeçalho que parou de reserializar o esquema por linha levou a **17,0 µs (2,61× no total)**; a carga em lote pela rede foi de **25.985 → 39.287 linhas/s**. Ordenar as chaves em si valia 1,06× antes do cache e vale **1,19×** depois — está medido em `docs/DESEMPENHO.md` §4.1, com a garantia que teria de ser rebaixada para implementá-lo |
