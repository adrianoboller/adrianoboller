...
- as **molduras** dos **vinte e uma** erros (`erro.corrompido` …
  `erro.config_ambiguo`) — o prefixo do `Display`, com `{detalhe}` no lugar
  da parte variável. Vinte e uma é o número de braços do `chave_do_erro`
  (`mensagens.rs`), que é exaustivo sobre o `PhxError`: variante nova não
  compila sem entrar ali. Dizia «doze» desde antes de a família crescer —
  número digitado numa prosa envelhece calado, e a última a chegar foi a
  `erro.config_ambiguo` (5002, 24/09/2026, pedido 481);
