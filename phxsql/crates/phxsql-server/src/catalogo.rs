...
pub const OPERACOES: &[Operacao] = &[
    ...
    Operacao {
        nome: "sql",
        apelidos: &[],
        resumo: "Traduz um SELECT simples — e INSERT, UPDATE e DELETE por \
                 chave única — para as operações do protocolo e os executa \
                 pelo MESMO portão de permissão. UPDATE e DELETE são três \
                 passos por dentro (buscar, ler, atualizar/excluir com a \
                 linha mesclada e a versão); uma linha por INSERT. Também \
                 atende os comandos de rotina no dialeto do MySQL(R): \
                 CREATE/DROP TRIGGER e PROCEDURE, CALL e SHOW \
                 TRIGGERS/PROCEDURES — criar, excluir e listar exigem \
                 administrar; CALL roda com o poder de quem chama. Ver \
                 docs/SQL.md §6 e docs/TRIGGERS.md.",
        parametros: &[
            opc(
                "database",
                "string",
    ...
    Operacao {
        ...
        parametros: &[
            ...
            opc("para", "string", "o id do nó a quem este pulso se destina -- entra na prova"),
            opc("quando", "integer", "o carimbo de quem manda, em ms; fora da janela de 60 s a prova não vale"),
            opc("nonce", "string", "hexadecimal sorteado por pulso; repetido na janela, o pulso é recusado"),
            opc(
                "prova",
                "string",
                "HMAC-SHA256 que amarra este pulso à identidade de quem o manda (pedido 278).                  A chave sai do Diffie-Hellman entre a estática deste nó e o `chave_do_fio` do                  destinatário -- os dois lados a derivam sozinhos, e não há nada a distribuir.                  Sem ela o pulso ainda passa, para não quebrar nó de versão anterior, até                  `cluster.exigir_prova_do_pulso` ser ligado ou aquele nó provar uma vez",
            ),
        ],
