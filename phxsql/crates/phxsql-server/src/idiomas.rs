...
pub const FABRICA_TELA: &[TextoDeFabrica] = &[
    ...
    texto!("tela.g_ligado_relogio_jobs_nao_subiu", "ligado — mas o relógio de jobs não subiu neste arranque: ", "activé — mais l'horloge des tâches n'a pas démarré à ce lancement : ", "enabled — but the jobs clock didn't come up on this start: ", "attivato — ma l'orologio dei job non è partito in questo avvio: ", "aktiviert — aber die Job-Uhr ist bei diesem Start nicht hochgefahren: ", "activado — pero el reloj de trabajos no arrancó en este inicio: "),
    texto!("tela.g_gravado_relogio_jobs_nao_subiu", "gravado — mas o relógio de jobs não subiu neste arranque: ", "enregistré — mais l'horloge des tâches n'a pas démarré à ce lancement : ", "saved — but the jobs clock didn't come up on this start: ", "salvato — ma l'orologio dei job non è partito in questo avvio: ", "gespeichert — aber die Job-Uhr ist bei diesem Start nicht hochgefahren: ", "grabado — pero el reloj de trabajos no arrancó en este inicio: "),
    ...
    texto!("tela.st_telemetria_o_que_faz", "o que o servidor está fazendo agora — séries, atividades e threads, ", "ce que le serveur fait en ce moment — séries, activités et threads, ", "what the server is doing right now — series, activity and threads, ", "cosa sta facendo il server ora — serie, attività e thread, ", "was der Server gerade tut — Reihen, Aktivität und Threads, ", "lo que el servidor está haciendo ahora — series, actividad e hilos, "),
