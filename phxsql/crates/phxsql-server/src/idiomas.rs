...
pub const FABRICA_TELA: &[TextoDeFabrica] = &[
    ...
    texto!("tela.tx_isolamento_b", "**Não é ANSI SERIALIZABLE** e não se chama assim: não há leitura repetível, e a transação não vê as próprias escritas.", "**Ce n'est pas ANSI SERIALIZABLE** et cela ne s'appelle pas ainsi : pas de lecture répétable, et la transaction ne voit pas ses propres écritures.", "**It is not ANSI SERIALIZABLE** and is not called that: there is no repeatable read, and a transaction does not see its own writes.", "**Non è ANSI SERIALIZABLE** e non si chiama così: non c'è lettura ripetibile, e la transazione non vede le proprie scritture.", "**Es ist nicht ANSI SERIALIZABLE** und heißt auch nicht so: kein Repeatable Read, und die Transaktion sieht ihre eigenen Schreibvorgänge nicht.", "**No es ANSI SERIALIZABLE** y no se llama así: no hay lectura repetible, y la transacción no ve sus propias escrituras."),
    ...
    texto!("tela.tx_sem_ler_o_proprio", "**Ler o que a própria transação escreveu.** Uma consulta dentro dela não vê o que ela mesma inseriu: o dado está em RAM, e não no disco de onde a leitura vem.", "**Lire ce que la transaction elle-même a écrit.** Une requête à l'intérieur ne voit pas ce qu'elle a inséré : la donnée est en RAM, pas sur le disque.", "**Reading what the transaction itself wrote.** A query inside it does not see what it inserted: the data is in RAM, not on the disk the read comes from.", "**Leggere ciò che la transazione stessa ha scritto.** Una query al suo interno non vede ciò che ha inserito: il dato è in RAM, non sul disco.", "**Lesen, was die Transaktion selbst schrieb.** Eine Abfrage darin sieht nicht, was sie einfügte: die Daten liegen im RAM, nicht auf der Platte.", "**Leer lo que la propia transacción escribió.** Una consulta dentro no ve lo que ella misma insertó: el dato está en RAM, y no en el disco."),
    ...
    texto!("tela.ia_fk_declarada", "**A chave estrangeira do PhxSql é DECLARADA, e não imposta.** O motor guarda a declaração e as telas a usam — mas ele **não confere** a referência na hora de gravar.", "**La clé étrangère de PhxSql est DÉCLARÉE, pas imposée.** Le moteur garde la déclaration et les écrans s'en servent — mais il **ne vérifie pas** la référence à l'écriture.", "**PhxSql's foreign key is DECLARED, not enforced.** The engine keeps the declaration and the screens use it — but it **does not check** the reference when writing.", "**La chiave esterna di PhxSql è DICHIARATA, non imposta.** Il motore conserva la dichiarazione e le schermate la usano — ma **non verifica** il riferimento in scrittura.", "**Der Fremdschlüssel von PhxSql ist DEKLARIERT, nicht erzwungen.** Die Engine hält die Deklaration, die Bildschirme nutzen sie — sie **prüft** die Referenz beim Schreiben **nicht**.", "**La clave foránea de PhxSql es DECLARADA, no impuesta.** El motor guarda la declaración y las pantallas la usan — pero **no comprueba** la referencia al grabar."),
    texto!("tela.ia_fk_declarada2", "Quem promete integridade que não existe entrega um estrago com nome bonito.", "Promettre une intégrité qui n'existe pas, c'est livrer un dégât avec un joli nom.", "Whoever promises integrity that does not exist delivers damage with a pretty name.", "Chi promette un'integrità che non esiste consegna un danno con un bel nome.", "Wer Integrität verspricht, die es nicht gibt, liefert einen Schaden mit hübschem Namen.", "Quien promete una integridad que no existe entrega un estrago con nombre bonito."),
