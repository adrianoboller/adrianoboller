...
pub const FABRICA_TELA: &[TextoDeFabrica] = &[
    ...
    texto!("tela.g_prompt_restaurar_registro", "Restaurar o registro {rowid}?\n\nMotivo (fica no .reason):", "Restaurer l'enregistrement {rowid} ?\n\nMotif (reste dans le .reason) :", "Restore record {rowid}?\n\nReason (stays in the .reason):", "Ripristinare il record {rowid}?\n\nMotivo (resta nel .reason):", "Datensatz {rowid} wiederherstellen?\n\nGrund (bleibt in der .reason):", "¿Restaurar el registro {rowid}?\n\nMotivo (queda en el .reason):"),
