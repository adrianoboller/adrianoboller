...
def escrever(d):
    ...
    t.l("   ausências é sequência, não esquecimento: sem subconsulta")
    t.l("   correlacionada não há `EXISTS` para pedir. A ordem está no")
    t.l("   `docs/PENDENCIAS.md`. (O exemplo que esta frase dava antes — o")
    t.l("   nível de isolamento acima de `READ COMMITTED` — deixou de ser")
    t.l("   ausência em 16/09/2026: é a leitura repetível pela trava, pedida.)")
