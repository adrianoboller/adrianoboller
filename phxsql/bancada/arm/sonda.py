...
tok = cfg.get("token", "")
s = socket.create_connection(("127.0.0.1", int(__import__("os").environ.get("PORTA","6992"))), timeout=30)
f = s.makefile("rw")
def call(d):
...
print("  inseridas: %d de 50, em %.0f ms sob emulacao (%.1f ms/linha)" % (n, dt, dt / 50))
